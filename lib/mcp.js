...
function createMcp({ store, client, baseUrl = '' }) {
  async function call(name, args = {}) {
    ...
    if (name === 'campaign_reports_set_view') return { view: store.setView(args.report_id == null ? null : String(args.report_id)) };
  ...
  async function handle(message) {
    ...
    try {
      if (message?.method === 'initialize') return { jsonrpc: '2.0', id, result: { protocolVersion: '2025-03-26', capabilities: { tools: { listChanged: false } }, serverInfo: { name: 'cosmise-campaign-reports', version }, instructions: 'The active Symposium Agent is the analyst. Read campaign_reports_get_bootstrap and campaign_reports_get_state first, clarify material scope, use only question-specific bounded reads, validate and complete the report, then select it with campaign_reports_set_view. Production access is private and read-only.' } };
