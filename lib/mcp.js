...
const TOOLS = [
  ...
  { name: 'campaign_reports_heartbeat', description: 'Record a throttled, platform-backed signal that the Agent is still actively working on a report.', inputSchema: { type: 'object', properties: { report_id: { type: 'string' } }, required: ['report_id'] } },
...
].map((tool) => ({ ...tool, annotations: { readOnlyHint: !['campaign_reports_set_view','campaign_reports_start','campaign_reports_heartbeat','campaign_reports_save_markdown','campaign_reports_complete','campaign_reports_fail','campaign_reports_share','campaign_reports_revoke_share'].includes(tool.name), destructiveHint: tool.name === 'campaign_reports_revoke_share' } }));
...
function handoffFor() { return 'Your report is ready in Cosmise Campaigns.'; }
...
function createMcp({ store, client, baseUrl = '' }) {
  async function call(name, args = {}) {
    ...
    if (name === 'campaign_reports_set_view') {
      ...
      const view = reportId == null ? store.setView(null) : store.setAgentView(reportId);
      const report = reportId ? store.rawReport(reportId) : null;
      const selected = Boolean(reportId && view.active_report_id === reportId);
      return { view, selected, ...(selected && report?.status === 'ready' ? { chat_handoff: { reply_exactly: handoffFor(report), duplicate_report_in_chat: false } } : {}) };
    }
    if (name === 'campaign_reports_start') {
      const requestId = String(args.request_id || '').trim();
      const existing = requestId ? store.reportForRequest(requestId, true) : null;
      if (existing) return { report: existing, view: store.setAgentView(existing.id), idempotent_replay: true };
    ...
    const view = store.setAgentView(reportId);
    if (name === 'campaign_reports_heartbeat') {
      const report = store.rawReport(reportId);
      if (['ready', 'failed'].includes(report.status)) return { report: store.report(reportId, false), view, active: false };
      const activity = store.upsertActivity({
        call_id: `agent-heartbeat:${reportId}`,
        report_id: reportId,
        operation: 'agent_work',
        title: 'Working now',
        status: 'running',
        detail: 'Your Agent is actively working on this report'
      });
      return { report: store.report(reportId, false), view, activity, active: true };
    }
