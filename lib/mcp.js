...
function createMcp({ store, client, baseUrl = '' }) {
  async function call(name, args = {}) {
    ...
    if (name === 'campaign_reports_set_view') {
      ...
      return { view, ...(report?.status === 'ready' ? { chat_handoff: { reply_exactly: 'Your report is ready in Cosmise Campaigns.', duplicate_report_in_chat: false } } : {}) };
    }
    ...
    if (name === 'campaign_reports_complete') {
      ...
      return { report, chat_handoff: { after_select_and_verify_reply_exactly: 'Your report is ready in Cosmise Campaigns.', duplicate_report_in_chat: false } };
    }
