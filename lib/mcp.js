...
const TOOLS = [
  ...
  { name: 'campaign_reports_start', description: 'Start a local Markdown report for one campaign question.', inputSchema: { type: 'object', properties: { title: { type: 'string' }, question: { type: 'string' } }, required: ['question'] } },
...
].map((tool) => ({ ...tool, annotations: { readOnlyHint: !['campaign_reports_start','campaign_reports_save_markdown','campaign_reports_complete','campaign_reports_fail','campaign_reports_share','campaign_reports_revoke_share'].includes(tool.name), destructiveHint: tool.name === 'campaign_reports_revoke_share' } }));

const BOOTSTRAP = Object.freeze({
  purpose: 'Create evidence-backed Markdown reports from private, credential-scoped, read-only campaign data.',
  production_boundary: { credential_owner: 'trusted app backend', mode: 'read', raw_production_tools_exposed: false, production_mutations: false },
  workflow: ['campaign_reports_start', 'bounded report reads', 'campaign_reports_save_markdown', 'campaign_reports_validate', 'campaign_reports_complete'],
  report_rules: ['State date, currency, attribution model and event-source scope.', 'Separate observed facts from interpretation and recommendations.', 'Include a method and limitations section.', 'Never include credentials or raw upstream responses.'],
  chart_format: 'Use fenced campaign-chart JSON with type=bar|line and matching labels/values arrays.',
  sharing: 'Sharing is an explicit, expiring snapshot action and never performs live data reads.'
});
...
function createMcp({ store, client, baseUrl = '' }) {
  async function call(name, args = {}) {
    ...
    if (name === 'campaign_reports_read_mapping_health') {
      const status = await client.read('mapping_health', args.query, reportId);
      const candidates = args.include_candidates ? await client.read('mapping_candidates', args.query, reportId) : null;
  ...
  async function handle(message) {
    ...
    try {
      if (message?.method === 'initialize') return { jsonrpc: '2.0', id, result: { protocolVersion: '2025-03-26', capabilities: { tools: { listChanged: false } }, serverInfo: { name: 'cosmise-campaign-reports', version }, instructions: 'Read campaign_reports_get_bootstrap first. Production access is private and read-only.' } };
