...
const AGENT_BOOTSTRAP = Object.freeze({
  ...
  invocation: {
    context_tool: 'get_app_agent_context',
    call_tool: 'call_app_tool',
    app_id: 'cosmise-campaigns',
    rule: 'Never guess the managed port or call the public app URL as an Agent API.'
  },
  api_boundaries: {
    production: {
  ...
  required_start: [
    ...
    'Call campaign_reports_start only after the question is sufficiently bounded.'
  ],
  ...
  completion: [
    ...
    'Read state again and verify the intended report is ready and selected.',
...
const AGENT_INSTRUCTIONS = [
  ...
  'Use SYM-Node get_app_agent_context with app_id=cosmise-campaigns, then call every local operation through call_app_tool. Never guess the managed port.',
  ...
  'Start one report, then use only the bounded reads needed for the question. Do not mechanically reproduce a generic attribution comparison.',
  ...
  'Save with the current revision, validate, repair errors, complete, select the report, then verify it is ready and visible.',
