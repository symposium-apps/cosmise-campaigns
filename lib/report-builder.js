...
function dateRangeFromQuestion(question, now = new Date()) {
  ...
  for (let month = 0; month < MONTHS.length; month += 1) {
    const range = text.match(new RegExp(`\\b${MONTHS[month]}\\s+(\\d{1,2})(?:st|nd|rd|th)?\\s*(?:through|to|[-–—])\\s*(?:${MONTHS[month]}\\s+)?(\\d{1,2})(?:st|nd|rd|th)?(?:,\\s*|\\s+)(20\\d{2})\\b`));
    if (!range) continue;
    const year = Number(range[3]);
    const firstDay = Number(range[1]);
    const lastDay = Number(range[2]);
    const daysInMonth = new Date(Date.UTC(year, month + 1, 0)).getUTCDate();
    if (firstDay < 1 || lastDay < firstDay || lastDay > daysInMonth) continue;
    const start = new Date(Date.UTC(year, month, firstDay));
    const end = new Date(Date.UTC(year, month, lastDay));
    return { start_date: dateIso(start), end_date: dateIso(end), label: `${formatDate(start)} to ${formatDate(end)}` };
  }
  for (let month = 0; month < MONTHS.length; month += 1) {
...
function buildReport({ question, range, contextRead, sourceRead, capabilityRead, models, results, mappingRead, diagnosticsRead, trendRead }) {
  ...
  if (!selected.length) {
    ...
    const totals = models.map((model) => {
      const values = totalsFor(resultByModel[model.key]);
      return `| ${tableText(model.label)}${isTotalCredit(model) ? '*' : ''} | ${money(values.spend)} | ${money(values.display_revenue ?? values.revenue)} | ${decimal(values.orders)} | ${number(values.display_roas ?? values.roas).toFixed(2)}× |`;
    }).join('\n');
    const markdown = `# ${title}

## Executive answer

The authorized Campaigns scope returned no campaign rows with spend for **${range.label}**. There is no evidence in this result set to rank campaigns, calculate a meaningful campaign CPA, or recommend budget movement.

## Scope

- **Question:** ${tableText(question)}
- **Organisation scope:** ${tableText(orgName)}
- **Analysis period:** ${range.label}
- **Currency:** ${currency}
- **Attribution models checked:** ${models.length}

## Model totals

| Attribution model | Spend | Attributed revenue | Conversions / credit | ROAS |
|---|---:|---:|---:|---:|
${totals}

## Tracking and mapping limitations

- No campaign-level rows with spend were available in the bounded read, so no winner or loser is claimed.
- Provider spend and ClickHouse-attributed revenue may be absent because the selected organisation, period, or connected ad accounts do not contain matching data.
- Mapping and identity diagnostics cannot substitute for missing campaign performance rows, and no mapping was written.

## Three recommended actions

1. **Confirm the organisation scope.** Verify that this profile is connected to the campaign account intended by the question.
2. **Confirm the reporting window.** Check that provider and attribution data exist for ${range.label} before changing budgets.
3. **Do not infer zero performance from missing rows.** Re-run after correcting scope or dates, then compare attribution models before making a campaign decision.

## Method and limitations

The automatic workflow used only the app's allowlisted, read-only Campaigns operations. It queried the same campaign cohort across ${models.length} available attribution models, received no campaign rows with spend, and therefore omitted the ranking chart rather than fabricating values. No campaign, mapping, extraction rule, alias, budget, customer, or production setting was changed.
`;
    return { title, markdown, summary: { strongest: null, stable: null, threshold, selected: [], totalRows: 0, currency, range, noData: true } };
  }
  ...
  const mappingStatusAvailable = Boolean(mappingRead?.status);
...
${mappingStatusAvailable ? `- Unassigned attribution contained **${money(unassignedRevenue)} revenue** and **${decimal(unassignedOrders)} order credit** in this window.` : '- Aggregate mapping coverage was unavailable from the upstream read endpoint. Exact-candidate diagnostics were retained, and no mapping was written.'}
