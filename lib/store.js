...
class ReportStore {
  ...
  setAgentView(id) {
    const target = this.rawReport(id);
    if (!target) throw new Error('Report not found.');
    const active = this.rawReport(this.state.view.active_report_id);
    if (active && active.id !== target.id && String(active.created_at || '') > String(target.created_at || '')) {
      return { ...this.snapshot().view, requested_report_id: target.id, selected: false, superseded_by_report_id: active.id };
    }
    return { ...this.setView(target.id), requested_report_id: target.id, selected: true };
  }
